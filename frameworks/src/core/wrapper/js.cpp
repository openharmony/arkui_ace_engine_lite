...
{
    return CallJSFunction(func, context, args, size);
}
...
JSValue JSObject::Keys(JSValue target)
{
    return jerry_get_object_keys(target);
}

JSValue JSObject::Get(JSValue target, JSValue key)
{
...
char *JSArray::GetString(JSValue array, uint32_t index)
{
    JSValue value = JSArray::Get(array, index);
    char *content = JSString::Value(value);
    JSRelease(value);
    return content;
}
