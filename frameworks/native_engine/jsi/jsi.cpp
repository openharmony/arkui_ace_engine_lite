...
void JSI::CallFunction(JSIValue funcObj, JSIValue thisVal, const JSIValue *argv, uint8_t argc)
{
    ...
    if ((argv == nullptr) || argc == 0) {
        jerry_value_t ret = jerry_call_function(jFunc, jThis, nullptr, 0);
        if (jerry_value_is_error(ret)) {
            HILOG_ERROR(HILOG_MODULE_ACE, "JSI:CallFunction error returned!");
        }
    ...
    jerry_value_t ret = jerry_call_function(jFunc, jThis, jArgv, argc);
    if (jerry_value_is_error(ret)) {
        HILOG_ERROR(HILOG_MODULE_ACE, "JSI:CallFunction error returned!");
    }
    jerry_release_value(ret);
...
{
    JsiErrorType jsiType = static_cast<JsiErrorType>(jerryType);
    if ((jsiType <= JsiErrorType::JSI_ERROR_INVALID) ||
        (jsiType >= JsiErrorType::JSI_ERROR_MAX)) {
